...
rollRange = [63801, 63850]
resultId = "11074878"
fileName = "BScSem1.xlsx"
...
var result = document
  .getElementsByClassName('c5')[1]
  .children[0].firstElementChild.innerHTML.split('<br>')[0];
var fail = true;
var aw = true;
...
var maxmarks = {
  ...
  Botany: 150,
  Zoology: 150,
  'Drug Abuse Problem, Management and Prevention': 0,
};
var subjects = [];
var marks = [];
...
for (var i = 1; i < rows.length - 1; ++i) {
  ...
  if (subn == 'Comp')
    if (parseInt(columns[1].innerText < 16)) result += subn + ',';
    else if (subn == 'ENV')
      if (parseInt(columns[1].innerText < 16)) result += subn + ',';
...
for i in range(rollRange[0],rollRange[1]+1):
    b.get("https://results.pupexamination.ac.in/t8/results/results.php?rslstid="+resultId+"&ROLL="+str(i)+"&submit=Submit")
