...
for (i = 0; i < a.length - 5; ++i) {
	...
	if (is_year) {
		if (subn.indexOf(course) != -1) {
			if (sem != "") {
				if (subn.indexOf("sem-" + sem) != -1 || subn.indexOf("sem - " + sem) != -1 || subn.indexOf("semester-" + sem) != -1 || subn.indexOf("semester - " + sem) != -1)
					if (helper_text != "") {
						if (subn.indexOf(helper_text) != -1) r.push([i, subn, yearn, a[i].querySelector("a").href]);
					} else
	...
	if (is_year) {
		if (subn.indexOf(course) != -1) {
			if (sem != "") {
				if (subn.indexOf("sem-" + sem) != -1 || subn.indexOf("sem - " + sem) != -1 || subn.indexOf("semester-" + sem) != -1 || subn.indexOf("semester - " + sem) != -1)
					if (helper_text != "") {
						if (subn.indexOf(helper_text) != -1) console.log(i, subn, yearn, "\n" + a[i].querySelector("a").href);
					} else
